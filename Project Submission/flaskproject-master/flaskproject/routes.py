...
def updateProject(project_id):
    ...
    if form.validate_on_submit():
        ...
        if len(membersList)>=1 and status==1:
            ...
            project.team.name=form.teamName.data
            for index, entry in enumerate(membersList):
                student = Student.query.filter_by(name=entry['memberName'],email=entry['memberEmail'],phone=entry['memberPhone'],cls=entry['memberClass']).first()
                if student!=None:
                    if student.team_id!=project.team_id:
                        flash(entry['memberName'] + ' already exists in another project.', 'danger')
                        return redirect(url_for('trackProjects', data=project.code))
                    else:
                        student.name=entry['memberName']
                        student.email=entry['memberEmail']
                        student.phone=entry['memberPhone']
                        student.cls=entry['memberClass']
                else:
                    new_student = Student(
                                team_id=project.team_id,
                                name=entry['memberName'],
    ...
    return render_template('ProjectRegistration.html', title='Update Project', form=form, students=students )
